...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        # ==================== Classification Head ====================
        # Add image-level classification head for HGG/LGG grading
        # Use bottleneck features from middle block
        self.use_cls_head = False  # Will be set externally
        # # Gradient detachment for staged training。为了可以缓解梯度干扰，不然分类头已经疯了
        self.cls_detach_features = True  
        bottleneck_channels = channel_mult[-1] * model_channels
        self.cls_head = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(0.3),
            nn.Linear(bottleneck_channels // 2, 2),  # Binary: LGG=0, HGG=1
        )
        # ============================================================
    ...
    def forward(self, x, timesteps, y=None):
        ...
        :param timesteps: a 1-D batch of timesteps.
        :param y: an [N] Tensor of labels, if class-conditional.
        :return: (seg_logits, cls_logits, calib_map) if use_cls_head else (seg_logits, calib_map)
        """
        assert (y is not None) == (
            self.num_classes is not None
        ), "must specify y if and only if the model is class-conditional"
        ...
        # Extract bottleneck features for classification before decoding
        h_bottleneck = self.middle_block(h, emb)
        
        # Classification head: use bottleneck features for image-level grading
        cls_logits = None
        if self.use_cls_head:
            # Detach features to prevent classification gradients from affecting segmentation
            if self.cls_detach_features:
                cls_logits = self.cls_head(h_bottleneck.detach())
            else:
        ...
        # Return format: (seg_logits, cls_logits, calib_map)
        if self.use_cls_head:
