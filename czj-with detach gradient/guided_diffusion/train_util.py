        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            if self.use_cls_head and "cls_logits" in losses and "grade_labels" in losses:
                ...
                if self.uncertainty_loss is not None:
                    ...
                    loss_seg_stable = torch.clamp(loss_seg, 0, 10.0)  # Clamp to prevent explosion
            ...
            # Log losses
            logger.logkv("loss_seg", loss_seg.item())
            logger.logkv("loss_total", total_loss.item())
            self.epoch_metrics['loss_seg'].append(loss_seg.item())
            self.epoch_metrics['loss_total'].append(total_loss.item())
