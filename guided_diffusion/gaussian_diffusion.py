...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if isinstance(model_output, tuple) and len(model_output) == 3:
            model_output, cls_logits, cal = model_output
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None, grade_labels=None, use_cls_head=False):
        """
        Compute training losses for a single timestep with optional classification.
        ...
        :param grade_labels: [N] tensor of grade labels (0=LGG, 1=HGG) for classification.
        :param use_cls_head: whether to include classification loss.
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            ...
            # Model forward pass - now returns (seg_output, cls_logits, cal) if use_cls_head
            model_forward_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
            
            if use_cls_head and len(model_forward_output) == 3:
                model_output, cls_logits, cal = model_forward_output
                terms["cls_logits"] = cls_logits  # Store for return
            else:
                model_output, cal = model_forward_output
                cls_logits = None
            ...
            if use_cls_head and cls_logits is not None and grade_labels is not None:
                # Classification loss will be computed in train_util.py using custom losses
                terms["grade_labels"] = grade_labels
